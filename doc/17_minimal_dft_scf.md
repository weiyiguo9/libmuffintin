...
`libmuffintin-runtime` provides the single `muffintin` binary and an ordinary Rust library boundary suitable for a later Python binding. Its `CheckpointPhysics` is a checkpoint/IO/orchestration shell: it normalizes checkpoint data, constructs `MaterialKernel`, delegates DFT/SCF physics to that kernel, emits restart checkpoints, and retains the product-space input bridges. Runtime also retains the MLDUMP and CoQui writers. The binary is not named after DFT because the same versioned task registry can acquire THC or other workflows without creating another executable.
...
| `frozen-checkpoint` | The exact checkpoint anchor, retained as the FLEUR/SPEX regression route |

`atomic` expands a scalar $l$ recipe into its signed $\kappa$ partners and uses exact $2j+1$ degeneracies when folding back to scalar. Full first variation keeps the partner energies distinct. `band-cog` projects $d=P_{\mathrm{site}}C$ and evaluates the full selected muffin-tin quadratic form $d^\dagger S_{\mathrm{MT}}d$, including nonorthogonal radial cross terms; occupation, k weight, and state degeneracy are multiplied exactly once by the generator. `band-cog` and `fermi-offset` run in a bounded inner basis-refinement loop after occupations but before density synthesis, against the same outer-iteration potential. Every outer iteration starts again from the immutable recipe and checkpoint provisional anchor, not the previous iteration's generated energy.
...
Both routes return $V_0=(v_\uparrow+v_\downarrow)/2$ and $\mathbf B_{xc}=(v_\uparrow-v_\downarrow)\hat{\mathbf m}/2$, without an extra factor of two or $\mu_B$, and report the total-energy contraction $\int(nV_0+\mathbf m\cdot\mathbf B_{xc})$. In the interstitial, analytic Fourier derivatives supply the density jet and a deterministic midpoint grid performs the nonlinear forward/inverse transform at every grid point, including the smooth plane-wave continuation inside the spheres. The interstitial region enters as the analytic step function truncated to the density layout, $\tilde\Theta(\mathbf r)=\sum_{\mathbf G\in\mathcal L}\Theta_{\mathbf G}e^{i\mathbf G\cdot\mathbf r}$, synthesized on the same grid: the energy contractions weight each point by $\tilde\Theta$, and the operator-facing potential is the grid product $\tilde\Theta V_{xc}$, so the potential coefficients are the exact derivative of the discrete energy. The surviving $\mathbf G=0$ coefficient makes the grid sum reproduce the exact interstitial volume, and the region no longer depends on which grid points fall inside a sphere. Local spin densities that finite Fourier or harmonic truncation drives below zero are clamped to zero before the point kernel, in both regions. Inside each sphere, a deterministic angular rule, quartic radial interpolation, and fourth-order Cartesian differences supply the same point-kernel contract.
...
Checkpoint V2 stores shared geometry and radial-basis metadata plus either a frozen Pauli potential or a restart pair containing $n,m_x,m_y,m_z$ and $V_0,B_x,B_y,B_z$. Checkpoint V1 remains readable and normalizes exactly to V2: scalar data maps to $V_0$ with zero $\mathbf B$, while up/down data maps to $V_0=(V_\uparrow+V_\downarrow)/2$, $B_z=(V_\uparrow-V_\downarrow)/2$, and zero transverse fields. `CheckpointPhysics` uses header-based version dispatch and one normalized V2 path, translates that IO representation once when constructing `MaterialKernel`, and converts accepted states back to restart checkpoints without moving checkpoint types into `libmuffintin-dft`.
...
The amplitudes are initial-guess devices, not occupied molecular orbitals; there are no cross-atom amplitude products. Their reciprocal support is chosen within half the density cutoff, and the caller's exact `RegionalFieldLayout` must contain all pair differences. This gives a nonnegative finite Fourier representation instead of relying on clipping a negative Gibbs oscillation. The XC transform clamps any residual negative local spin density to zero, so the initial guess only needs to be positive where it carries charge. The atomic path does not infer the density layout from a k mesh, orbital basis, or `CompiledBasis`. `RegionalFieldLayout::new` accepts an ordered reciprocal-index list, while `from_g_cutoff` enumerates the reciprocal sphere from an explicit `InverseBohr`.
