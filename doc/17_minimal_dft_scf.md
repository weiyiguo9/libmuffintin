...
The amplitudes are initial-guess devices, not occupied molecular orbitals; there are no cross-atom amplitude products. Their reciprocal support is chosen within half the density cutoff, and the caller's exact `RegionalFieldLayout` must contain all pair differences. This gives a nonnegative finite Fourier representation instead of relying on clipping a negative Gibbs oscillation. When that mathematically positive field is reconstructed for XC, a negative spin eigenvalue is projected to zero only within the coefficient-scale roundoff bound accumulated over the three-transform FFT chain; any larger negative density remains a typed error. The atomic path does not infer the density layout from a k mesh, orbital basis, or `CompiledBasis`. `RegionalFieldLayout::new` accepts an ordered reciprocal-index list, while `from_g_cutoff` enumerates the reciprocal sphere from an explicit `InverseBohr`.
