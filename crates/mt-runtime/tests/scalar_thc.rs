...
use muffintin::{
    RankPolicy, SCALAR_RADIAL_U, SCALAR_RADIAL_UDOT, ScalarProductInput, ScalarThcCandidates,
    ScalarThcEngine, ScalarThcError, ScalarThcGrid, ScalarThcPoint, ScalarThcRegion, ScalarThcSpec,
    SnapshotDftPhysics, build_scalar_thc,
};
...
fn parent_grid(input: &ScalarProductInput) -> ScalarThcGrid {
    ...
    ScalarThcGrid::new(
        ...
        vec![
            ScalarThcPoint {
                ...
                weight: 0.35,
                region: ScalarThcRegion::MuffinTin {
                    site: 0,
                    radial_index: 0,
                },
            },
            ...
            ScalarThcPoint {
                ...
                weight: 0.8,
                region: ScalarThcRegion::Interstitial,
            },
            ScalarThcPoint {
...
fn spec_all(n_mu: usize) -> ScalarThcSpec {
    spec_all_engine(n_mu, ScalarThcEngine::FullColumnPivotedQr)
}

fn spec_all_engine(n_mu: usize, engine: ScalarThcEngine) -> ScalarThcSpec {
    ScalarThcSpec {
        spin: 0,
        rank: RankPolicy::Exact { n_mu },
        candidates: ScalarThcCandidates::All,
        engine,
    }
}
...
fn finite_q_pair_density_uses_stored_positive_wrap_not_global_umklapp() {
    ...
    let spec = ScalarThcSpec {
        ...
        candidates: ScalarThcCandidates::Indices(vec![0]),
        engine: ScalarThcEngine::FullColumnPivotedQr,
    };
...
fn multi_q_allq_l2_fits_heterogeneous_weights_with_independent_residual() {
    ...
    let engines = [
        ScalarThcEngine::FullColumnPivotedQr,
        ScalarThcEngine::FullPivotedCholesky,
    ];
...
fn scalar_thc_rejects_empty_slice_or_partition_mismatch() {
    ...
    let bad = ScalarThcGrid::new(other, Provenance::default(), grid.points().to_vec()).unwrap();
    assert!(matches!(
    ...
    let zero_candidate = ScalarThcSpec {
        spin: 0,
        rank: RankPolicy::Exact { n_mu: 1 },
        candidates: ScalarThcCandidates::Indices(vec![1]),
        engine: ScalarThcEngine::FullColumnPivotedQr,
    };
    assert!(matches!(
    ...
    off_shell[0].region = ScalarThcRegion::MuffinTin {
        site: 0,
        radial_index: 0,
    };
    let bad_shell = ScalarThcGrid::new(
        input.source.partition.clone(),
        Provenance::default(),
        off_shell,
    )
