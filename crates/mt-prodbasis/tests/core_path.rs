...
fn pair_vertex_rejects_mismatched_transfer_q() {
    ...
    assert!(matches!(
        ...
        Err(muffintin_prodbasis::mpb::MpbError::TransferQMismatch)
    ));
}
...
fn pair_vertex_rejects_same_sites_with_different_cell_volume() {
    ...
    assert!(matches!(
        ...
        Err(muffintin_prodbasis::mpb::MpbError::PartitionMismatch)
    ));
}
...
fn pair_vertex_rejects_permuted_or_relabelled_raw_pair_support() {
    ...
    assert!(matches!(
        ...
        Err(muffintin_prodbasis::mpb::MpbError::InterstitialPairSupportMismatch)
    ));
    ...
    assert!(matches!(
        ...
        Err(muffintin_prodbasis::mpb::MpbError::Product(
...
fn pair_vertex_rejects_absent_mt_pair() {
    ...
    assert!(matches!(
        pair_vertex(&source, &raw, &auxiliary, spec),
        Err(muffintin_prodbasis::mpb::MpbError::UnknownMtPair { .. })
    ));
}
...
fn raw_pair_support_survives_independent_auxiliary_g_cut() {
    ...
    assert!(matches!(
        ...
        Err(muffintin_prodbasis::mpb::MpbError::UnknownInterstitialPair { g }) if g == [1, 0, 0]
    ));
}
