...
fn gram_q_index_is_validated_before_whitening() {
    ...
    assert!(matches!(
        error,
        muffintin_prodbasis::thc::ThcError::GramQIndex {
            expected: 0,
            actual: 1
        }
    ));
    ...
    assert!(matches!(
        ...
        Err(muffintin_prodbasis::thc::ThcError::GramColumnOrder(1))
    ));
    assert!(matches!(
        shifted.require_context(1, q, shifted.layout),
        Err(muffintin_prodbasis::thc::ThcError::GramShape { index: 1, .. })
    ));
}
...
fn three_selectors_distinguish_finite_q_action_at_identical_nmu() {
    ...
    let grid = ToyGrid {
        ...
        points,
        weights: vec![1.0; n],
        regions: vec![muffintin_prodbasis::InterpolationRegion::Uniform; n],
    };
