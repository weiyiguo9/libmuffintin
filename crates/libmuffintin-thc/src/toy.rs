...
use libmuffintin_core::{
    Bohr, InterstitialGeometry, Sphere, VolumeBohr3, complex_spherical_harmonic,
};
use libmuffintin_product::{InterpolationRegion, ProductPartition};
...
pub fn mt_uniform_grid(n: usize, shift: crate::select::UniformShift) -> ToyGrid {
    let offset = match shift {
        crate::select::UniformShift::Origin => [0.0; 3],
        crate::select::UniformShift::Half => [0.5; 3],
        crate::select::UniformShift::Random { seed } => {
...
pub fn mt_adaptive_grid(nrad: usize, nang: usize, ninter: usize) -> ToyGrid {
    ...
    let uniform = mt_uniform_grid(ninter, crate::select::UniformShift::Half);
...
fn orthonormalize(
    ...
    for ik in 0..n_k {
        ...
        let (values, vectors) = crate::linalg::hermitian_eigensystem(&overlap, n_orb)?;
        if values[0] < 1.0e-10 {
...
pub fn toy_coulomb_gram(
    ...
    let expected = crate::error::checked_storage_len(&[n_g, n])?;
    ...
    let gram_len = crate::error::checked_storage_len(&[n, n])?;
...
pub fn values_fourier(
    ...
    let expected = crate::error::checked_storage_len(&[n_points, n_col])?;
    ...
    crate::error::validate_quadrature_weights(&grid.weights)?;
    let n_g = g_cart.len();
    let mut out = vec![Complex64::default(); crate::error::checked_storage_len(&[n_g, n_col])?];
    for (g_index, g) in g_cart.iter().enumerate() {
        for p in 0..n_points {
            let phase = Complex64::from_polar(
                1.0,
...
pub fn approximate_pair_fourier(
    ...
    let expected_zeta = crate::error::checked_storage_len(&[n_g, n_mu])?;
    ...
    let expected_rows = crate::error::checked_storage_len(&[n_mu, n_col])?;
...
pub fn relative_gram_frobenius(
    ...
    let denom = crate::linalg::frobenius(reference.data());
    if denom <= 1.0e-30 {
        return Ok(0.0);
    }
    Ok(crate::linalg::frobenius(&diff) / denom)
}
...
pub fn compare_candidate_eri_action(
    ...
    let pair_denom = crate::linalg::frobenius(reference_pair_fourier).max(1.0e-30);
    ...
    let pair_fourier = crate::linalg::frobenius(&pair_diff) / pair_denom;
    ...
    let exact_frob = crate::linalg::frobenius(exact_gram.data()).max(1.0e-30);
    let eri_frobenius = crate::linalg::frobenius(&diff) / exact_frob;
