...
fn evaluate_noncollinear_xc_point(
    ...
    // Finite Fourier and harmonic truncations produce slightly negative local
    // spin densities in vacuum and near sphere surfaces; the kernel sees
    // vacuum there, as in every production LAPW code.
    let mut jet = split_local_spin_jet(charge, projected);
    for spin_density in &mut jet.rho {
        *spin_density = spin_density.max(0.0);
    }
...
mod tests {
    ...
    #[test]
    fn negative_spin_densities_are_clamped_to_vacuum() {
        let zero = FieldJet::value(0.0);
        let vacuum = evaluate_noncollinear_xc_point(
            XcFunctional::LdaPw92,
            NoncollinearXcRoute::LocalSpinFrame,
            FieldJet::value(-2.0e-3),
            [zero; 3],
        )
        .unwrap();
        assert_eq!(vacuum.potential, [0.0; 4]);
        assert_eq!(vacuum.energy_density, 0.0);
        assert_eq!(vacuum.density_potential, 0.0);

        // n = 0.1, m_z = 0.3 splits into spin densities (0.2, -0.1).
        let polarized = evaluate_noncollinear_xc_point(
            XcFunctional::LdaPw92,
            NoncollinearXcRoute::LocalSpinFrame,
            FieldJet::value(0.1),
            [zero, zero, FieldJet::value(0.3)],
        )
        .unwrap();
        let reference = evaluate_xc_point(
            XcFunctional::LdaPw92,
            DensityJet2 {
                rho: [0.2, 0.0],
                gradient: [[0.0; 3]; 2],
        ...
        .unwrap();
        let up = reference.potential[0].get();
        let down = reference.potential[1].get();
        assert_eq!(
            polarized.potential,
            [0.5 * (up + down), 0.0, 0.0, 0.5 * (up - down)]
        );
