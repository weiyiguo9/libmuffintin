...
use muffintin_coulomb::{
    CoreCoreFockAction, CoreCoreFockError, CoreCoreFockShell, CoreCoreFockTrace,
...
/// Controls for the M3a core-core inner loop at one immutable local potential.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct CoreCoreFixedPotentialSpec {
...
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct CoreCoreShellResidual {
...
#[derive(Clone, Debug, PartialEq)]
pub struct CoreCoreRelaxationIteration {
    pub iteration: usize,
    pub shells: Vec<CoreCoreShellResidual>,
...
/// Converged M3a sidecar and a fresh CC trace generated from its final radials.
#[derive(Clone, Debug, PartialEq)]
pub struct CoreCoreFixedPotentialResult {
    pub orbitals: CoreShellOrbitals,
    pub diagnostics: Vec<CoreCoreRelaxationIteration>,
...
/// Relax one site's closed/uniform-mu core shells against core-core exchange.
///
...
/// builds all actions from the same input shell set and solves all shells
/// before replacing any of them. Only consecutive actions are mixed.
pub fn relax_core_core_at_fixed_potential(
    initial: &CoreShellOrbitals,
    spec: CoreCoreFixedPotentialSpec,
) -> Result<CoreCoreFixedPotentialResult, CoreCoreRelaxationError> {
    ...
    for iteration in 1..=spec.max_iterations {
        let fresh = build_sidecar_core_core_action(&current)?;
        let action = mix_core_core_actions(
            ...
            .enumerate()
        {
            ...
            if !energy_change.is_finite() {
                return Err(CoreCoreRelaxationError::NonFiniteEnergyChange {
                    iteration,
                    shell: shell_index,
                    value: energy_change,
                });
            ...
            if !radial_residual.is_finite() {
                return Err(CoreCoreRelaxationError::NonFiniteRadialResidual {
                    iteration,
                    shell: shell_index,
            ...
            shell_residuals.push(CoreCoreShellResidual {
                shell_index,
                state: shell.state,
            ...
            && maximum_radial_residual <= spec.radial_tolerance
        {
            ...
            return Ok(CoreCoreFixedPotentialResult {
                orbitals: current,
                diagnostics,
                final_cc_trace,
            });
    ...
    Err(CoreCoreRelaxationError::NotConverged {
...
fn validate_relaxation_spec(
    spec: CoreCoreFixedPotentialSpec,
) -> Result<(), CoreCoreRelaxationError> {
    if !spec.action_mixing.is_finite() || spec.action_mixing <= 0.0 || spec.action_mixing > 1.0 {
        return Err(CoreCoreRelaxationError::InvalidActionMixing(
            spec.action_mixing,
        ));
    }
    if !spec.energy_tolerance.get().is_finite() || spec.energy_tolerance.get() <= 0.0 {
        return Err(CoreCoreRelaxationError::InvalidEnergyTolerance(
            spec.energy_tolerance.get(),
        ));
    }
    if !spec.radial_tolerance.is_finite() || spec.radial_tolerance <= 0.0 {
        return Err(CoreCoreRelaxationError::InvalidRadialTolerance(
            spec.radial_tolerance,
        ));
    }
    if spec.max_iterations == 0 {
        return Err(CoreCoreRelaxationError::InvalidMaximumIterations);
    }
    Ok(())
}

fn validate_core_core_sidecar(sidecar: &CoreShellOrbitals) -> Result<(), CoreCoreRelaxationError> {
    if sidecar.provenance.extended_potential.len() != sidecar.extended_mesh.len() {
        return Err(CoreCoreRelaxationError::PotentialLength {
    ...
    if sidecar.provenance.solve_specs.len() != sidecar.shells.len() {
        return Err(CoreCoreRelaxationError::SolveSpecCount {
        ...
        .enumerate()
    {
        if shell.state != spec.state {
            return Err(CoreCoreRelaxationError::ShellState {
                shell: shell_index,
                orbital: shell.state,
...
) -> Result<muffintin_coulomb::CoreCoreFockResult, CoreCoreRelaxationError> {
...
fn uniform_mu_occupation(
...
) -> Result<f64, CoreCoreRelaxationError> {
    let CoreShellOccupations::MuResolved(occupations) = &shell.occupations else {
        return Err(CoreCoreRelaxationError::ExplicitCollinear { shell: shell_index });
    };
    ...
    {
        return Err(CoreCoreRelaxationError::MagneticChannels { shell: shell_index });
    }
    ...
    {
        return Err(CoreCoreRelaxationError::OpenShell { shell: shell_index });
    }
    Ok(reference)
}
...
/// Invalid M3a fixed-potential input, radial failure, or bounded nonconvergence.
#[derive(Debug, Error)]
pub enum CoreCoreRelaxationError {
...
mod tests {
    ...
    #[test]
    fn core_core_relaxation_converges_at_one_immutable_provenance_potential() {
        ...
        let result = relax_core_core_at_fixed_potential(
            &initial,
            CoreCoreFixedPotentialSpec {
                action_mixing: 0.5,
