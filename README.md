...
The M-Kb DFT contract and M-Kc orbital-configuration V2 extension are closed across the library, ordered TOML workflow, single executable, and versioned snapshot/restart boundary, but they are not yet cross-code accepted or production validated. M-L1 is the scalar frozen-snapshot product-input boundary, M-L2 is the scalar mixed-product/selected-vertex bridge, M-L3 is the scalar AllQL2 interpolation-point/$\zeta$ seam, M-L4 is the sampled-$\zeta$ Coulomb bridge, M-L5a is the parallel Dirac PP/QQ IR and MPB primitive, M-L5b is the frozen full-first-variation spinor product-input boundary, M-L5c is the selected-band spinor mixed-product bridge, M-L5d is the spinor all-$q$ THC plus sampled-$\zeta$ Coulomb bridge, M-L6a is the versioned MLDUMP v1 HDF5 header/geometry/mesh interchange, M-L6b2 materializes frozen scalar product/THC/Coulomb objects into that schema through a streaming writer, and M-L6c2 materializes the corresponding frozen spinor objects; M-L6d1 writes a CoQui-native scalar full-BZ Cholesky file from those scalar objects and is not MLDUMP or CoQui full compatibility; they are not SPEX/material acceptance and do not complete M-L. `read_spex_snapshot_hdf` reads SPEX-owned `spex.snapshot_hdf` v1 frozen fields; `materialize_snapshot_v2` builds Snapshot V2 only with an explicit signed-$\kappa$ recipe. It is not MLDUMP, does not invent SPEX kappa, and does not treat omitted $B$ as zero. `SnapshotDftPhysics::new_spex_material` preserves the imported scalar Koelling–Harmon source equation, binds each caller-owned recipe channel and energy to the exact runtime `ScfBasis` request/resolved record, and only then authorizes a target Dirac $P,Q$ solve from the imported $V_0$ monopole; raw Koelling–Harmon `SnapshotDftPhysics::new` still rejects full first variation. M-L7 is a bounded material-acceptance demo note ([`doc/20_sm_dy_full_spinor_material_demo.md`](doc/20_sm_dy_full_spinor_material_demo.md)), not a release: the hashed SPEX `b45d9b9` Sm fcc artifact executes target Dirac product input, bounded full-$q$ MPB, rank-one QRCP and pivoted-Cholesky THC, sampled Coulomb, and spinor MLDUMP roundtrip, while the Dy bcc lane remains blocked by the absence of an honest libmuffintin-consumable Snapshot V2. SPEX `exchange_hdf` and the CoQui scalar Cholesky adapter do not consume spinor full-first-variation MLDUMP. Snapshot V2 carries $n,m_x,m_y,m_z$ and $V_0,B_x,B_y,B_z$ for frozen noncollinear input and restart; Snapshot V1 remains readable through exact scalar or up/down normalization. Focused and one-site end-to-end gates do not replace the planned Si/SrVO3 scalar, Pt/Au second-variation SOC, collinear and noncollinear magnetic, and cross-code tetrahedron-DOS fixtures. The older Cu frozen-potential one-meV gate and producer-specific FLEUR conversion also remain outside the current acceptance evidence, so a `v0.1` release tag is not claimed. v0.2 closes with the in-flight M-L THC-chain work and then M-Kd, an atomic-superposition Snapshot V2 generator (free-atom radial SCF, overlapped neutral-atom densities on the muffin-tin geometry) that gives the native DFT SCF a cold start from structure alone; today the frozen-potential start must come from an authored or producer-supplied snapshot, which remains the Dy M-L7 producer blocker class. After that, the next planned milestone is M-M ([`doc/21_python_binding_and_export_schema.md`](doc/21_python_binding_and_export_schema.md)): a thin PyO3/maturin Python binding (v0.3) that exports the frozen M-L scalar and spinor product/THC/Coulomb boundaries as versioned NumPy structures for auxiliary-basis experiments in the separate backend-neutral `pymuffintin` package (this repository ships only the binding), with no new physics and no SCF, symmetry, importer, or wheel scope. The previously planned v0.3 and v0.4 contents each move back one release: the remaining M-L7 material-acceptance lane is a v0.4 candidate, and the former v0.4 scope follows in v0.5. The SPEX `spex.snapshot_hdf` v1 importer route (`read_spex_snapshot_hdf` plus `materialize_snapshot_v2`) is in-flight v0.2 M-L work, not a deferred item.
