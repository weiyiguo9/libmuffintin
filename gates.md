...
| G-CTF-S1 | `ctf-rs` | the eight sparse drivers and five `test_sparse.py` semantics of `plans/ctf-rs/plan.v3.md` section 3, batches S1a to S1c, class R | same | same | ctf-rs `docs/validation.md` section per batch; WSL 1/2/4 once per batch; the S1 set once native at 1/2/4 | accepted, evt-0066; after C1 |
