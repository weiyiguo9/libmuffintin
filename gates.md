...
| G-CTF-S1 | `ctf-rs` | the eight sparse drivers and five `test_sparse.py` semantics of `plans/ctf-rs/plan.v3.md` section 3, batches S1a to S1c, class R | same | same | ctf-rs `docs/validation.md` section per batch; WSL 1/2/4 once per batch; the S1 set once native at 1/2/4 | HANDOFF, evd-1018 (10 of 13 at ctf-rs 0b65b90); closes under plan.v4 at evd-1019 |
| G-CTF-S1d | `ctf-rs` | `sparse_einsum_hadamard` and `sparse_scaled_expression` after the sparse Hadamard-index elimination of `plans/ctf-rs/plan.v4.md` section 4, class R | dense Rust computation of the same expression; the source step is pinned `contraction.cxx:5417-5527` | sum(abs(diff)) < 1e-14, the source file's own `allclose` | ctf-rs `docs/validation.md` "S1d and S1 close"; WSL 1/2/4 once, native 1/2/4 once | accepted, evt-0069 |
| G-CTF-S1-CK7 | `ctf-rs` | `upstream_checkpoint_sparse` at the source default n=7, class R | the source six-decimal text round trip | norm2(v−u) < 1e-7·n·n·0.1·n = 3.43e-6 | same; WSL 1/2/4 once, native 1/2/4 once | accepted, evt-0069; replaces the n=3 HANDOFF of evd-1018 |
| G-CTF-S1-AMG | `ctf-rs` | `upstream_algebraic_multigrid` at WSL 1/2/4 on the plan.v4 close tree, class R | the twice-smoothed Jacobi residual | rnorm < rnorm_alt | same; inside the one S1-set WSL run | accepted, evt-0069; native 1/2/4 already passed at evd-1018 |
