...
| G-CTF-A1 | `ctf-rs` | every gating driver after the audit remediation of `plans/ctf-rs/plan.v5.md`, class R; a changed result fails the refactor | pinned cc4s/ctf `f69cbb46` | upstream tolerance, unchanged | the full `scripts/acceptance-wsl.sh` with the S1 targets added, once at 1/2/4; native build once; the S1 set once native at 1/2/4 | proposed, evt-0073 |
